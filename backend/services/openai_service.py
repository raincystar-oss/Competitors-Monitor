...
class OpenAIService:
    ...
    def __init__(self):
        ...
        logger.info(f"  Base URL: {settings.openai_base_url or 'https://api.openai.com/v1 (default)'}")
        logger.info(f"  Модель текста: {settings.openai_model}")
        logger.info(f"  Модель vision: {settings.openai_vision_model}")
        logger.info(f"  API ключ: {'*' * 10}...{settings.openai_api_key[-4:] if settings.openai_api_key else 'НЕ ЗАДАН'}")
...
Формат ответа (строго JSON):
{
    ...
    "informative_content": 7,
    "informative_content_analysis": "Краткий анализ информативности контента",
    "summary": "Краткое резюме анализа"
}

Важно:
- informative_content от 0 до 10
...
- Будь конкретен и практичен в рекомендациях"""
        ...
        try:
            ...
            result = CompetitorAnalysis(
                ...
                informative_content=data.get("informative_content", 0),
                informative_content_analysis=data.get("informative_content_analysis", ""),
...
Формат ответа (строго JSON):
{
    ...
    "design_score": 7,
    "design_analysis": "Анализ качества дизайна и композиции",
...
- Оценивай: цветовую палитру, типографику, композицию, UX/UI элементы"""
    ...
    async def analyze_parsed_content(
        ...
        logger.info(f"  Полный текст: {len(page_content) if page_content else 0} символов")
        ...
        if page_content:
            clipped_content = page_content[:12000]
            content_parts.append(f"Полный контент страницы:\n{clipped_content}")
        if paragraph:
    ...
    async def analyze_website_screenshot(
        ...
        if page_content:
            clipped_content = page_content[:12000]
            context_parts.append(f"Полный текст страницы:\n{clipped_content}")
        if first_paragraph:
