...
class MainWindow(QMainWindow):
    ...
    def show_results(self, analysis: dict, result_type: str = "text"):
        ...
        if result_type == "text" or result_type == "parse":
            if "informative_content" in analysis:
                info_score = analysis.get("informative_content", 0)
                info_frame = QFrame()
                info_frame.setObjectName("resultBlock")
                info_layout = QVBoxLayout(info_frame)

                info_title = QLabel("⭐ Оценка информативности контента")
                info_title.setObjectName("sectionTitle")

                info_value = QLabel(f"{info_score}/10")
                info_value.setStyleSheet("font-size: 32px; font-weight: bold; color: #22d3ee;")

                info_layout.addWidget(info_title)
                info_layout.addWidget(info_value)

                if analysis.get("informative_content_analysis"):
                    info_desc = QLabel(analysis["informative_content_analysis"])
                    info_desc.setWordWrap(True)
                    info_desc.setStyleSheet("color: #94a3b8;")
            ...
            if "design_score" in analysis:
                ...
                if analysis.get("design_analysis"):
                    design_desc = QLabel(analysis["design_analysis"])
                    design_desc.setWordWrap(True)
                    design_desc.setStyleSheet("color: #94a3b8;")
                    design_layout.addWidget(design_desc)
